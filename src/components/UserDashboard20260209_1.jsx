...
export default function DashboardSinglePage({ onLogout }) {
  ...
  // ✅ NEW: raw designation rows from new endpoint (includes ex_cader_court)
  ...
  // ✅ Overall split filter
  // "ALL" | "IN_FIELD" | "EX_CADRE"
  const [selectedCadre, setSelectedCadre] = useState("ALL");

  // District search + sort
  const [districtQuery, setDistrictQuery] = useState("");
  const [districtSort, setDistrictSort] = useState("division_az"); // ✅ default
  ...
  // ✅ IMPORTANT CHANGE:
  // For cadre != ALL: DO NOT silently fallback to overall method
  // so district counts behave exactly like designation side.
  const fetchOverallDistrictTiles = async (cadre = "ALL") => {
    console.log(fetchOverallDistrictTiles);
    if (cadre !== "ALL") {
      const data = await Api.getDistrictPostingCountoverAllByCadre(cadre);
      return (data || []).map((x) => ({
  ...
  const fetchDistrictByDesignation = async (designationDesc, cadre = "ALL") => {
    console.log("On Tile Click", cadre);
    if (cadre !== "ALL") {
      const data = await Api.getDistrictPostingCountByDesignationAndCadre(
        designationDesc,
        cadre,
      );
      return (data || []).map((x) => ({
  ...
  const fetchDesignationsByDistrict = async (districtName) => {
    console.log("fetchDesignationsByDistrict", cadre);
    const data =
  ...
  const showAll = async () => {
    ...
    const aggAll = aggregateDesignationTiles(rawDesignationRows, "ALL");
    setBaseDesignationTiles(aggAll);
    setDesignationTiles(aggAll);

    setLoadingRight(true);
    try {
      const d2 = await fetchOverallDistrictTiles("ALL");
  ...
  const onDesignationClick = async (labelOrAll) => {
    ...
    try {
      ...
      const list = await fetchDistrictByDesignation(labelOrAll, selectedCadre);
